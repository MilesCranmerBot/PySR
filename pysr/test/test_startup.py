...
class TestStartup(unittest.TestCase):
    ...
    def test_type_spec_predict_from_pickle_in_fresh_process(self):
        if platform.system() == "Windows":
            self.skipTest("Fresh-process TypeSpec test incompatible with Windows")

        with tempfile.TemporaryDirectory() as tmpdirname:
            model_file = Path(tmpdirname) / "model.pkl"
            fit_result = subprocess.run(
                [
                    ...
                    textwrap.dedent(f"""
                        ...
                        model = PySRRegressor(
                            type_spec=spec,
                            operators={{1: ["identity_string_fresh(x::String) = x"]}},
                            elementwise_loss=(
                                "string_loss_fresh(x::String, y::String) = "
                                "x == y ? 0.0 : 1.0"
                            ),
                            niterations=1,
                        ...
                        model.fit(X, y)
                        with open({str(model_file)!r}, "wb") as file:
            ...
            self.assertEqual(fit_result.returncode, 0, fit_result.stderr.decode())
            ...
            predict_result = subprocess.run(
                [
                    ...
                    textwrap.dedent(f"""
                        ...
                        with open({str(model_file)!r}, "rb") as file:
            ...
            self.assertEqual(
                predict_result.returncode, 0, predict_result.stderr.decode()
            )
